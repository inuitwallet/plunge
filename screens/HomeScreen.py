...
class HomeScreen(Screen):
    ...
    def set_exchange_spinners(self):
        ...
        self.exchange_buy_side_label.text = ''
        self.exchange_sell_side_label.text = ''
        self.exchange_balance_label.text = ''
        self.exchange_ask_rate_label.text = ''
        self.exchange_bid_rate_label.text = ''
        self.exchange_efficiency_label.text = ''
    ...
    def get_stats(self, dt):
        ...
        start_threads = {'pool': True, 'exchange': True, 'personal': True}
        for thread in thread_list:
            if thread.name == 'pool_thread':
                start_threads['pool'] = False
            if thread.name == 'exchange_thread':
                start_threads['exchange'] = False
            if thread.name == 'personal_thread':
                start_threads['personal'] = False
        if start_threads['pool']:
            self.PlungeApp.logger.debug("build and start pool thread")
            pool = Thread(target=self.get_pool_stats, name='pool_thread')
            pool.start()
        if start_threads['exchange']:
    ...
    def get_pool_stats(self):
        ...
        if req.resp_status != 200:
            self.PlungeApp.logger.debug("not saving pool stats")
            return
    ...
    def get_exchange_stats(self):
        ...
        req.wait()
        if req.resp_status != 200:
            self.PlungeApp.logger.debug("not saving exchange stats")
            return
        # calculate the exchange totals
        # ask
        unique_ask_orders = []
        for sample in self.stats[self.primary_exchange][self.primary_currency]['ask']['orders']:
            for order in sample:
                if round(order['amount'], 4) in unique_ask_orders:
                    continue
                unique_ask_orders.append(round(order['amount'], 4))
        self.primary_exchange_total_ask = 0
        for amount in unique_ask_orders:
            self.primary_exchange_total_ask += amount
        # bid
        unique_bid_orders = []
        for sample in self.stats[self.primary_exchange][self.primary_currency]['bid']['orders']:
            for order in sample:
                if round(order['amount'], 4) in unique_bid_orders:
                    continue
                unique_bid_orders.append(round(order['amount'], 4))
        self.primary_exchange_total_bid = 0
        for amount in unique_bid_orders:
            self.primary_exchange_total_bid += amount
    ...
    def get_personal_stats(self):
        ...
        if not self.PlungeApp.active_exchanges or not self.PlungeApp.active_currencies:
            ...
            return
        for exchange in self.PlungeApp.active_exchanges:
            self.PlungeApp.logger.debug("get stats for %s" % exchange)
            ...
            for api_set in api_keys:
                if api_set['exchange'] == exchange:
                    public = api_set['public']
                    self.PlungeApp.logger.info("Get Personal Stats for %s > %s" % (exchange, public))
                    ...
                    if req.resp_status != 200:
                        self.PlungeApp.logger.debug("not saving personal stats for %s" % public)
                        continue
                    
                    self.user[exchange]['balance'] += self.personal_result['balance']
                    self.user[exchange]['num_keys'] += 1
                    self.user[exchange]['efficiency'] += self.personal_result['efficiency']
                    for currency in self.PlungeApp.active_currencies:
                        self.PlungeApp.logger.debug("saving personal stats for %s" % currency)
                        if currency in self.personal_result['units']:
                            for order in self.personal_result['units'][currency]['ask']:
                                self.user[exchange][currency]['ask_liquidity'] += order['amount']
                            for order in self.personal_result['units'][currency]['bid']:
                                self.user[exchange][currency]['bid_liquidity'] += order['amount']
                            self.user[exchange][currency]['ask_rate'] += self.personal_result['units'][currency]['rate']['ask']
                            self.user[exchange][currency]['bid_rate'] += self.personal_result['units'][currency]['rate']['bid']
                            ...
                            self.ensure_lists(self.exchange_total_liquidity, exchange, currency)
                            self.update_lists((self.user[exchange][currency]['ask_liquidity'] +
                                               self.user[exchange][currency]['bid_liquidity']),
                                              self.exchange_total_liquidity[exchange][currency])

                            self.ensure_lists(self.exchange_ask_rate, exchange, currency)
                            self.update_lists(round(float(self.user[exchange][currency]['ask_rate'] * 100), 4),
                                              self.exchange_ask_rate[exchange][currency])

                            self.ensure_lists(self.exchange_bid_rate, exchange, currency)
                            self.update_lists(round(float(self.user[exchange][currency]['bid_rate'] * 100), 4),
                                              self.exchange_bid_rate[exchange][currency])
                        else:
                            self.PlungeApp.logger.debug("%s not found in returned stats" % currency)

                    if self.user[exchange]['num_keys'] > 0:
                        self.user[exchange]['efficiency'] /= float(self.user[exchange]['num_keys'])

            # update the graph data lists
            self.PlungeApp.logger.debug("update personal graphs lists")
            self.ensure_lists(self.exchange_efficiency, exchange)
            self.update_lists((self.user[exchange]['efficiency'] * 100),
                              self.exchange_efficiency[exchange])
            self.ensure_lists(self.exchange_balance, exchange)
            self.update_lists(round(float(self.user[exchange]['balance']), 4),
    ...
    def personal_stats_error(self, req, result):
        self.PlungeApp.logger.warn('Personal stats failed. Returned %s' % req.resp_status)
    ...
    def calculations(self):
        ...
        self.PlungeApp.logger.debug("Calculations Finished")
    ...
    def show_exchange_info(self):
        ...
        data = [(self.PlungeApp.get_string('Total_Liquidity'), self.PlungeApp.get_string('No_Data')),
                (self.PlungeApp.get_string('Target'), self.PlungeApp.get_string('No_Data')),
        ...
        if self.primary_exchange in self.stats:
            if self.primary_currency in self.stats[self.primary_exchange]:
                ...
                data = [(self.PlungeApp.get_string('Total_Liquidity'), '%.4f / %.4f' % (self.primary_exchange_total_ask, self.primary_exchange_total_bid)),
                        (self.PlungeApp.get_string('Target'), '%.2f / %.2f' % (stats['ask']['target'], stats['bid']['target'])),
    ...
    def add_label(self, text):
        self.personal_info_bottom_layout.add_widget(Label(text=text))

    def load_personal_info(self, obj, value):
        print value
        self.add_label('Exchange')
        keys = json.load(open('api_keys.json'))
        for key in keys:
            if key['public'] == value:
                self.add_label(self.PlungeApp.get_string(key['exchange']))
